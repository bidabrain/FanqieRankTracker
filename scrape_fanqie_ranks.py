...
CHANNELS = {
    "female": {
        ...
        "output_prefix": "fanqie_female_new_ranks",
    },
    "male": {
        "init_url": "https://fanqienovel.com/rank/1_1_1141",
        "output_prefix": "fanqie_male_new_ranks",
    },
}
CHAR_SEQUENCE = [
...
def run_scraper(limit=30, sleep_sec=5, channel="female"):
    ...
    output_file = os.path.join(OUTPUT_DIR, f"{output_prefix}_{date_str}.json")
    state_file = os.path.join(OUTPUT_DIR, f"task_state_{channel}_{date_str}.json")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="番茄小说新书榜爬虫")
    parser.add_argument("--channel", choices=["female", "male"], default="female",
                        help="频道：female=女频（默认），male=男频")
    args = parser.parse_args()
    channel_label = "女频" if args.channel == "female" else "男频"
    print(f"开始执行番茄{channel_label}新书榜抓取计划...")
    run_scraper(limit=30, sleep_sec=5, channel=args.channel)
